...
open class SpriteSheet {
    ...
    init?(sheetImage: UIImage, info sInfo: SheetInfo) {
        ...
        let bytes = UnsafeMutableRawPointer.allocate(byteCount: memory.0, alignment: memory.1)
    ...
    deinit {
        ...
        if let data = imageData {
            _ = (sheetBytesCount * MemoryLayout<UInt8>.stride, MemoryLayout<UInt8>.alignment)
            data.deallocate()
        }
        imageData = nil
    }
